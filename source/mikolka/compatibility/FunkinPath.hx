...
class FunkinPath {
    ...
    public static function getPath(path:String,forceNative:Bool = false):String {
        ...
        if(curMod != null && curMod != '' && FileSystem.exists(#if mobile Sys.getCwd() + #end 'dlcs/$curMod/$path'))
            return #if mobile Sys.getCwd() + #end 'dlcs/$curMod/$path';
        else if (FileSystem.exists(#if mobile Sys.getCwd() + #end 'dlcs/$path')) return #if mobile Sys.getCwd() + #end 'dlcs/$path';
