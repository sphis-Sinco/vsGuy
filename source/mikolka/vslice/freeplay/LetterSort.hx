...
class LetterSort extends FlxTypedSpriteGroup<FlxSprite>
{
	...
	override function update(elapsed:Float):Void
	{
		...
		{ // ? changed controls mapping
			if (Controls.FREEPLAY_LEFT || (TouchUtil.overlaps(swipeBounds) && SwipeUtil.swipeLeft))
				changeSelection(-1);
			if (Controls.FREEPLAY_RIGHT || (TouchUtil.overlaps(swipeBounds) && SwipeUtil.swipeRight))
				changeSelection(1);
		}
	}
